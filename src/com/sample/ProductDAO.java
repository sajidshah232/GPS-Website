...
public class ProductDAO {
    ...
    public static void addProduct(Product p, Part part, Part p2, Part p3, ServletContext ctx) throws Exception {
        ...
        if (p2.getSubmittedFileName().length() > 1) {
            ps = con.prepareStatement("update products set photo2=? where prodid=?");
            ps.setString(1, id + "-2.jpg");
            ps.setString(2, id);
            ps.executeUpdate();
            fileCopy(ctx, p2, id + "-2.jpg");
        }
        if (p3.getSubmittedFileName().length() > 1) {
            ps = con.prepareStatement("update products set photo3=? where prodid=?");
            ps.setString(1, id + "-3.jpg");
            ps.setString(2, id);
            ps.executeUpdate();
            fileCopy(ctx, p3, id + "-3.jpg");
        }
        con.close();
    ...
    public static List<Product> getTopProducts() throws Exception {
        List<Product> list = new ArrayList<>();
        Connection con = Connect();
        ResultSet rs = null;
        rs = con.createStatement().executeQuery("SELECT * FROM products order by prodid limit 4");
        while (rs.next()) {
    ...
    public static Product findByID(String prodid) throws Exception {
        ...
        p.setPcat(rs.getString("pcat"));
        p.setPhoto(rs.getString("photo"));
        p.setPhoto2(rs.getString("photo2"));
        p.setPhoto3(rs.getString("photo3"));
        p.setPrice(rs.getFloat("price"));
        p.setFeatured(rs.getString("featured"));
        p.setInstock(rs.getString("instock"));
        p.setDisc_price(rs.getFloat("disc_price"));
